...
## Recommended Use (Notebook/Colab)
This project is designed to run as a Jupyter/Colab notebook.

1. Open the notebook in `notebooks/`.
2. Run cells in order (Cell‑0 → Cell‑11).
3. Use `-e` for evaluation outputs or `-d` for full debug outputs.
...
```
qPyCR/
├── notebooks/      # Colab/Jupyter notebook exports
├── scripts/        # Python exports (.py)
├── cells/          # Development cells
├── examples/       # Example datasets (add yours here)
└── PROJECT_OVERVIEW.md
```

## Install (optional, local)
Notebook/Colab is recommended. If you want to run locally:
```
